	...
	try {
		switch (intent) {
			case "create-room": {
				...
				return redirect(`/memory-optimization/room/${newRoom.id}`);
			}
			...
			case "join-room-secret": {
				...
				const data = await res.json();
				return redirect(`/memory-optimization/room/${data.id}`);
			}
			...
			case "dev-join-room-id": {
				...
				if (res.ok) return redirect(`/memory-optimization/room/${roomId}`);
